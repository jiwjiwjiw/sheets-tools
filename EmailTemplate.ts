...
class EmailTemplate {
    ...
    constructor(private params: EmailTemplateParams) {
        ...
        if(!sheet) {
            SpreadsheetApp.getUi().alert(`Tentative d'accès à la feuille inexistante "${params.sheetName}"`)
            return
        } 
    ...
    public addValidation() {
        ...
        validationHandler.add(new Validation(this.params.sheetName, 'A2:A', '', '', false, [
            'sujet',
            'titre',
            'sous-titre',
            'paragraphe',
        ...
        validationHandler.add(new Validation(this.params.sheetName, 'B2:B', '', '', false, this.params.conditions))
    }
    ...
    public insertData(html: string, data):string {
